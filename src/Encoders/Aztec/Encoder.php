...
namespace Barcoder\Encoders\Aztec;
	...
	public function encode($content, $eccPercent, $hint)
	{
		...
		switch ($hint) {
			...
			default:
				//throw azException::EncoderError('Unknown encoder');
                                throw new BarcoderException('Unknown encoder');
		}
		...
		if ($layers == $LAYERS_FULL) {
			//throw azException::EncoderError('Data too large');
                        throw new BarcoderException('Data to large');
		}
		...
		if ($compact) {
			...
			for ($i = 0, $j = 0; $i < intval($baseMatrixSize / 2) - 1; $i += 15, $j += 16) {
               ...
               for ($i = 0; $i < $w; ++$i) {
                   for ($s = 0; $s < $w; ++$s) {
                       if (!array_key_exists($s, $this->MATRIX[$i])) {
                           $this->MATRIX[$i][$s] = 0;
                       }
                       ksort($this->MATRIX[$s]);
                   }
               }
               $d = count($this->MATRIX[1]);

               for ($i = 0; $i < $w; ++$i) {
                   for ($s = 0; $s < $w; ++$s) {
                        $pixelGrid[] = $this->MATRIX[$s][$i];
                   }
               }
               $this->MATRIX = array_chunk($pixelGrid, $w);
//echo '<pre>';
//print_r(array_chunk($pixelGrid, $w));
//echo '</pre>';
               return array(
                   'g' => 'm',
                   'q' => array(4, 4, 4, 4),
	...
	private function generateCheckWords($stuffedBits, $totalSymbolBits, $wordSize)
	{
		...
		if ($messageSizeInWords == 0) {
			//throw azException::InvalidInput('No data bytes provided');
                        throw new BarcoderException('No data bytes provided');
		}

		if ($ecBytes == 0) {
			//throw azException::InvalidInput('No error correction bytes');
                        throw new BarcoderException('No error correction bytes');
		}
